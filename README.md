...
| `scrollbarOptions`   | `object` | `{}`    | Options passed to EnhancedScrollbar                                                           | —                                |
...
`prevButtonContent` and `nextButtonContent` options are rendered via `innerHTML`. Kaysa does **not** sanitize this input — it is your responsibility to ensure all content passed to these options is trusted.
